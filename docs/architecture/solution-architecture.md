...
| **State** | ledger contract | **SQLite / small Postgres** | **managed Postgres + RLS** |
...
**Schema migrations.** Authored with **Drizzle Kit** (TS-native; supports our two backends, PostgreSQL and SQLite), SQL checked into `packages/ledger/migrations`. CI **lints** migrations with **Atlas** (single Go binary, multi-arch) to flag destructive/irreversible changes and verify they apply on both dialects. **Not Liquibase/Flyway** — mature but JVM-based, a heavy dependency for a Node/TS stack and for self-host on small hardware (ADR-0017). Migrations **run automatically on startup behind a lock** (Postgres advisory lock / SQLite file lock) so only one migrator runs at a time.
...
- **Fresh install** (empty -> latest) on **both Postgres and SQLite**.
