...
| T5 restart-resume idempotency gate (the acceptance centerpiece) | ⬜ Pending (needs Docker host) | Handoff brief: `docs/design/0010-t3-t5-selfhost-verify-handoff.md` (seed Stalwart → pass N → `docker restart app` → pass 0 duplicates, cursor advanced; wire into the manual Spark e2e). |
