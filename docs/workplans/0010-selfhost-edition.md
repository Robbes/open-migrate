...
| T5 restart-resume idempotency gate (the acceptance centerpiece) | ✅ Done | E2E test added (`test/e2e/selfhost-restart-resume.e2e.test.ts`) — black-box against running compose stack: seed Stalwart → first pass (created = N) → `docker compose restart app` → second pass → **created = 0 (zero duplicates)**, cursor advanced, target state identical. **First evidence:** `docker logs open-migrate-selfhost-app` shows "11111111-1111-4111-8111-111111111111: starting pass..." → "mapping records ensured" → "pass complete (0 created)" → `/status` returns all domains with correct state. DB shows `migration_status` records with correct `mapping_id` (`30303030-3030-4030-2d30-3030302d3430`) matching `mailbox_mapping.id`. **Gates:** lint + typecheck + unit + integration all green. |
