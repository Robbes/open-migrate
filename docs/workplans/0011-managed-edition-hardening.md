...
|---|---|---|
| T1 runtime RLS enforcement (CRITICAL) | ✅ Done | **DB layer:** `0008_force_rls_enforcement.sql` (FORCE RLS) + `0009_create_app_user_role.sql` (non-owner `app_user`). **App layer:** `packages/ledger/src/db.ts` exports `withTenant()` helper using `SELECT set_config('app.current_tenant', $1, true)` for transaction-scoped context. **Tests:** `packages/ledger/src/rls.integration.test.ts` extended with 6 new tests proving: (1) tenant isolation between A and B, (2) cross-tenant INSERT fails, (3) fail-closed when no context, (4) cross-tenant UPDATE/DELETE prevented, (5) rollback on error. **API wiring:** `apps/api/src/middleware/auth.ts` exposes `withTenantDb()` wrapper; `apps/api/src/routes/tenants/index.ts` uses it for GET /api/tenants and GET /api/tenants/:id. **API test:** `apps/api/src/routes/tenants/tenants.integration.test.ts` added for HTTP-layer cross-tenant isolation proof. |
