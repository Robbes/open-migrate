...
| T7 managed compose stack + operator docs | 🟡 Partial (advanced) | **Landed:** demo **seed script** (`apps/api/src/scripts/seed-managed.ts` + `pnpm --filter @openmig/api seed:managed`) — two demo tenants (owner + connections + mailboxes + mapping), idempotent (fixed UUIDs + `ON CONFLICT DO NOTHING`), all writes through `withTenant()`, mints a demo owner JWT per tenant (no password-login endpoint yet); **`docs/operator-runbook.md`** (start/stop, seed, backup/restore §22.1, upgrade, tenant offboarding / GDPR erasure §17, operator can/cannot-see); **`deploy/compose/managed.env.example`**; and a **critical RLS wiring fix** in `managed.yml` — `api`+`worker` now connect via `APP_DATABASE_URL` (non-owner `app_user`) so RLS is actually enforced (the owner URL is superuser and bypasses RLS even under FORCE). Gates: lint + typecheck green. **Still open (needs a Docker host):** the `apps/api`/`apps/worker`/`apps/web` **Dockerfiles do not exist**, so `docker compose -f managed.yml up` can't build the app tier yet; the clean-`up` → DoD-journey run is **not verified end-to-end**; pin the `trigger.dev` image + reconcile the T3 v3/v4 SDK mismatch before wiring live tasks. |
