...
| T2 API routes: replace TODO shells with real persistence | ✅ In Progress - Billing & Members wired | **Billing routes** (apps/api/src/routes/billing/index.ts): Replaced in-memory `billingApi` with real Drizzle queries through `withTenantDb`. Routes updated: GET /usage (aggregates usage_metric rows by period), POST /usage (creates/updates individual metric rows for storage, egress, compute, api_calls), GET /usage/history (aggregates by period), POST /estimate (pure calculation, no DB), GET /invoices, GET /invoices/:id, POST /invoices/:id/pay, GET/POST /payment-methods, PATCH /payment-methods/:id/default. All use `withTenantDb` for RLS enforcement. **Members routes** (apps/api/src/routes/tenants/members.ts): Replaced TODO shells with real Drizzle queries. Routes updated: GET /:tenantId/members, POST /:tenantId/members (invite), GET /:tenantId/members/:memberId, PATCH /:tenantId/members/:memberId (with last-owner protection), DELETE /:tenantId/members/:memberId (with last-owner and self-removal checks). All use `withTenantDb` for RLS enforcement. **Integration tests created**: `apps/api/src/routes/billing/billing.integration.test.ts` and `apps/api/src/routes/tenants/members.integration.test.ts` with cross-tenant isolation tests for every route. Tests connect as `app_user` role to ensure RLS is enforced. Pending: dependency installation and test execution. |
