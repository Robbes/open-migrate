...
## Status — 2026-07-19 (update this block at the end of every session)

> **The foundation is now in place.** **T1–T4 are done and merged** (PRs #43–#50): runtime RLS
> enforcement, real API persistence, Trigger.dev jobs calling the real core (mail path), and
> usage metering from real runs. The **remaining epic is the last mile of the managed edition**:
> T5 (billing + Mollie test-mode e2e), T6 (web UI verified against the API), T7 (managed compose
> seed + operator runbook). Two T3 remainders are carried forward below (non-mail sync domains and
> the cutover/rollback jobs are still shells). `deploy/compose/managed.yml` exists (from 0006-G).
...
| T5 billing + Mollie test-mode end-to-end | ⬜ Pending | `@mollie/api-client@4` service (`apps/api/src/services/mollie/index.ts`) + webhook route (`apps/api/src/routes/billing/webhooks.ts`) exist, but the webhook is a **shell** — it logs the payment status and the invoice-status writes are commented out; **no invoice-generation job** aggregates `usage_metric` → `invoice` rows. Wire: invoice generation (period aggregation, ADR-0014 cost-recovery pricing) + webhook that verifies authenticity and drives the invoice/payment state machine to `paid`. |
| T6 web UI wired to the real API | 🟡 Partial | Vite React pages exist (`apps/web/src/pages/`: Login, Dashboard, Mappings, CreateMapping, MappingDetail, Billing, Tenants, Settings, OperatorDashboard) and the service layer (`apps/web/src/services/*`) already calls the real `/api` endpoints via `apiClient`. **Open:** the mapping wizard must submit a config `parseMappingConfig` accepts (shared schema), status pages must render ledger-derived state incl. verbatim errors, and per-page component tests + the DoD two-tenant journey are **unverified** against the compose stack. Note: `migration/nextjs-15` was **not** adopted — Vite stays (tag `archive/nextjs-15` preserves the RC work). |
| T7 managed compose stack + operator docs | ✅ Done | **Dockerfiles built and verified:** `apps/api/Dockerfile`, `apps/web/Dockerfile`, `apps/worker/Dockerfile` all build successfully (multi-stage, Node 24-slim, non-root, digest-pinned bases). **Compose stack verified:** `docker compose -f deploy/compose/managed.yml up` brings up Postgres → API + Web all healthy. **Seed script executed successfully** from within the API container: `docker exec open-migrate-api /app/node_modules/.bin/tsx /app/apps/api/src/scripts/seed-managed.ts` → two demo tenants created (Demo Tenant A — Acme Families: `a0000000-0000-4000-8000-000000000001`, Demo Tenant B — Bakerloo SMB: `b0000000-0000-4000-8000-000000000002`) with owner JWTs minted. **Database migrations applied:** All 12 migrations (0001-0012) executed successfully via `docker exec -i open-migrate-db psql -U openmigrate -d openmigrate < migration.sql`. **API authenticated:** `curl -H "Authorization: Bearer <jwt>" http://172.17.0.1:3001/api/tenants` returns tenant list with correct RLS enforcement. **Operator runbook:** `docs/operator-runbook.md` exists with start/stop/seed/backup/restore/tenant-offboarding procedures. **Gates:** `pnpm lint && pnpm typecheck && pnpm test && pnpm test:integration` all pass. **Docker hygiene:** Containers cleaned up after verification. |
