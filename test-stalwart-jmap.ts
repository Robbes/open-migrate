...
async function main() {
  ...
  // Try to create an admin account using the JMAP Account/set method
  // This should work without authentication for the first account
  console.log('\n=== Trying to create admin account via JMAP ===');
  try {
    const response = await fetch(`http://${host}:${port}/jmap/`, {
      method: 'POST',
      headers: {
